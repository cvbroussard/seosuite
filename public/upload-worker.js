/**
 * Upload SharedWorker — processes file uploads in a shared background thread.
 * Survives page navigation and refreshes. Dies only when ALL tabs close.
 *
 ...
 *   { type: "progress", items: [...], activeCount, doneCount, errorCount }
 */
...
function broadcast(msg) {
  for (const port of ports) {
    try { port.postMessage(msg); } catch { /* port may be dead */ }
  }
}
...
async function processNext() {
  ...
  try {
    if (next.sourceUrl) {
      ...
      // File arrives as ArrayBuffer + metadata (can't pass File objects to SharedWorker)
...
self.onconnect = function (e) {
  ...
  // Send current status to newly connected page
  broadcastStatus();
};
