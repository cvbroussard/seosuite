...
export function SeoAdminClient({ sites }: { sites: SiteInfo[] }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/admin/sites/${selectedSiteId}/search-console`).then(r => r.ok ? r.json() : { queries: [], pages: [] }),
    ])
      .then(([scoreData, searchData]) => {
  ...
  async function scoreAll() {
    setScoring("all");
    try {
      await fetch(`/api/admin/sites/${selectedSiteId}/page-scores`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "score_all" }),
      });
      const fresh = await fetch(`/api/admin/sites/${selectedSiteId}/page-scores`);
  ...
  async function verifySite() {
    ...
    setScoring("verify");
    try {
      const res = await fetch(`/api/admin/sites/${selectedSiteId}/search-console`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function syncSearchData() {
    ...
    try {
      ...
      if (data.success) {
        const fresh = await fetch(`/api/admin/sites/${selectedSiteId}/search-console`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted">PageSpeed scoring, Search Console, site verification</p>
        </div>
        <select
      ...
      {selectedSite && (
        <div className="flex items-center gap-3 text-[10px]">
          <span className="rounded bg-surface-hover px-2 py-1 text-muted">
            {selectedSite.customDomain || "No custom domain"}
          </span>
          <span className={`rounded px-2 py-1 ${selectedSite.gscProperty ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
            {selectedSite.gscProperty ? "Search Console verified" : "Search Console not verified"}
          </span>
          <span className="rounded bg-surface-hover px-2 py-1 text-muted">
            {selectedSite.scoreCount} pages scored
          </span>
          <span className="rounded bg-surface-hover px-2 py-1 text-muted">
            {selectedSite.searchCount} search rows
          </span>
        </div>
      )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={syncing || !selectedSite?.gscProperty}
                    className="rounded bg-accent px-4 py-1.5 text-xs font-medium text-white hover:bg-accent-hover disabled:opacity-50"
                    title={!selectedSite?.gscProperty ? "Verify Search Console first" : ""}
                  >
                ...
                <div className="rounded-xl border border-border bg-surface p-4 shadow-card">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-baseline justify-between py-1.5 border-b border-border">
                      ...
                      <span className="text-xs font-medium">{selectedSite?.customDomain || "—"}</span>
                    </div>
                    <div className="flex items-baseline justify-between py-1.5 border-b border-border">
                      <span className="text-[10px] text-muted">GSC Property</span>
                      <span className="text-xs font-medium">{selectedSite?.gscProperty || "Not set"}</span>
                    </div>
                    <div className="flex items-baseline justify-between py-1.5 border-b border-border">
                      <span className="text-[10px] text-muted">Meta Tag</span>
                      <span className={`text-xs font-medium ${selectedSite?.gscVerificationToken ? "text-success" : "text-muted"}`}>
                        {selectedSite?.gscVerificationToken ? "Injected" : "Not set"}
                      </span>
                    </div>
                    <div className="flex items-baseline justify-between py-1.5">
                      <span className="text-[10px] text-muted">Status</span>
                      <span className={`text-xs font-medium ${selectedSite?.gscProperty ? "text-success" : "text-warning"}`}>
                        {selectedSite?.gscProperty ? "Verified" : "Pending"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-border bg-surface p-4 shadow-card">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <button
                        onClick={verifySite}
                        disabled={scoring !== null || !selectedSite?.customDomain}
                        className="rounded bg-accent px-4 py-1.5 text-xs font-medium text-white hover:bg-accent-hover disabled:opacity-50"
                      >
                        {scoring === "verify" ? "Verifying..." : selectedSite?.gscProperty ? "Re-verify" : "Verify Domain"}
                      </button>
                    </div>
                    ...
                    {!selectedSite?.customDomain && (
                      <p className="text-[10px] text-warning">No custom domain configured. Provision one in Site Controls → Website tab first.</p>
                    )}
                  </div>
                </div>
              </div>
